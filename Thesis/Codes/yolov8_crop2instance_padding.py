...
#Add padding to make the image and the mask larger
def add_padding_with_transparency(image, mask, padding_percentage):
    
    #First getting the data types right
...
for filename in os.listdir(folder_path):
    ...
    #             print(class_label)
                ...
                if (class_label == find_text_between_brackets(filename)) and (results[0].boxes.boxes[i][4]> score_pct) :
                    ...
                    padded_image, padded_mask = add_padding_with_transparency(cropped_image, binary_mask_array, 0.2)
        
                    
                    cv2.imwrite(home_dir+'/PipelineImages/'+str(output_dir)+'/{}_segemented_image_invmask_{}.png'.format(filename.split(".")[0], i), cv2.bitwise_not(np.array(padded_mask)))
                    
                    # Save the cropped image
                    padded_image.save(home_dir+'/PipelineImages/'+str(output_dir)+'/{}_segemented_image_{}.png'.format(filename.split(".")[0], i), "PNG")
